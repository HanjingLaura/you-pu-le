"use client";

import { PianoKeys } from "@phosphor-icons/react";

type KeyChoice = {
  id: string;
  label: string;
  instrumentId: string;
};

const KEY_CHOICES: KeyChoice[] = [
  { id: "C", label: "C", instrumentId: "flute" },
  { id: "Bb", label: "bB", instrumentId: "soprano_sax" },
  { id: "Eb", label: "bE", instrumentId: "alto_sax" },
  { id: "F", label: "F", instrumentId: "horn_f" },
];

const INSTRUMENT_TO_KEY: Record<string, string> = {
  flute: "C",
  oboe: "C",
  trombone: "C",
  tuba: "C",
  violin: "C",
  viola: "C",
  cello: "C",
  guitar: "C",
  clarinet_bb: "Bb",
  soprano_sax: "Bb",
  tenor_sax: "Bb",
  trumpet_bb: "Bb",
  alto_sax: "Eb",
  bari_sax: "Eb",
  horn_f: "F",
};

type Props = {
  value: string;
  disabled?: boolean;
  onChange: (id: string) => void;
};

export function keyLabelForInstrument(instrumentId: string) {
  if (instrumentId === "piano") return "钢琴";
  const keyId = INSTRUMENT_TO_KEY[instrumentId] ?? "C";
  return KEY_CHOICES.find((item) => item.id === keyId)?.label ?? "C";
}

export function InstrumentPicker({ value, disabled, onChange }: Props) {
  const pianoOn = value === "piano";
  const selectedKey = pianoOn ? null : (INSTRUMENT_TO_KEY[value] ?? "C");

  return (
    <div className="key-picker" role="radiogroup" aria-label="乐器和调性">
      <button
        type="button"
        role="radio"
        aria-checked={pianoOn}
        aria-label="钢琴"
        className={`key-chip key-chip--piano ${pianoOn ? "is-selected" : ""}`}
        disabled={disabled}
        onClick={() => onChange("piano")}
      >
        <PianoKeys className="key-chip__piano" weight={pianoOn ? "fill" : "regular"} />
      </button>
      <span className="key-picker__rule" aria-hidden="true" />
      {KEY_CHOICES.map((choice) => {
        const checked = choice.id === selectedKey;
        return (
          <button
            key={choice.id}
            type="button"
            role="radio"
            aria-checked={checked}
            aria-label={`${choice.label} 调`}
            className={`key-chip ${checked ? "is-selected" : ""}`}
            disabled={disabled}
            onClick={() => onChange(choice.instrumentId)}
          >
            <span className="key-chip__label">{choice.label}</span>
          </button>
        );
      })}
    </div>
  );
}
